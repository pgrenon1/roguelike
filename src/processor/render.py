...
class RenderConsole(esper.Processor):
    ...
    def render_entity(self):
        for (rend, pos) in self.get_entities():
            ...
            self.scene.con.default_fg = rend.foreground_color
            # self.scene.con.default_bg = rend.bg
            self.scene.con.print_(
                x=pos.x,
                y=pos.y,
                string=rend.character,
...
class RenderPanel(esper.Processor, metaclass=Singleton):
    ...
    def __init__(self):
        ...
        self.msg_width = config.MESSAGE_WIDTH
        self.msg_height = config.MESSAGE_HEIGHT
        self.msg_x = config.MESSAGE_X
    ...
    def get_position_under_mouse(self):
        ...
        iterable = self.world.get_components(c.Position, c.Metadata)
...
class RenderTitle(esper.Processor):
    ...
    def process(self):
        ...
        self.scene.manager.root_console.print_(
            ...
            y=config.SCREEN_HEIGHT // 2 - 4,
            string=self.title,
        )

        self.scene.manager.root_console.print_(
            x=(config.SCREEN_WIDTH - len(self.author)) // 2,
...
class RenderCharacterScreen(esper.Processor):
    ...
    def process(self):
        ...
        y = config.SCREEN_HEIGHT // 2 - self.height // 2
        window.blit(
            dest=self.scene.manager.root_console,
            dest_x=x,
            dest_y=y,
